...
# 测试用例数据
register_test_data = [
    ("", "8888", "123456", "123456", "13812347863", "请输入正确手机号"),
    ("13877778888", "", "123456", "123456", "13812347863", "图像验证码错误"),
    ("13877776666", "8888", "123456", "131456", "13812347863", "两次密码不一致"),
    ("13877776666", "8888", "", "123456", "13812347863", "密码有效长度为"),
    ("13877776666", "8888", "123456", "", "13812347863", "两次密码不一致"),
    ("13812347863", "8888", "123456", "123456", "", "该手机号已注册"),
    ("13877778990", "8888", "123456", "123456", "13812347863", "13877778990")
]
...
class TestRegister:
    ...
    def setup_method(self):
        # 打开注册页面
        self.driver.get("http://192.168.249.129/index.php/Home/user/reg.html")
    ...
    # 测试用例方法
    @pytest.mark.parametrize("username, verify_code, password, password2, invite, expected_result", register_test_data)
    def test_register(self, username, verify_code, password, password2, invite, expected_result):
        ...
        # 使用driver自带的js库直接操作点击注册按钮（因为不使用js的话，程序运行到这里就直接报错无法点击注册按钮元素）
