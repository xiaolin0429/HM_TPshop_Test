...
# 测试用例数据
goods_test_data = [
    ("iPhone 15 Pro Max", "商品库存不足，剩余0,当前购物车已有0件"),
    ("iPhone 13 Pro Max", "-- 抱歉没找到您要搜索的商品，换个条件试试！--"),
    ("iPhone 14 Pro Max", "订单提交成功，我们将在第一时间给你发货！")
]
...
class TestOrder:
    ...
    # 下面是添加订单的操作步骤
    @pytest.mark.parametrize("goods, expected_result", goods_test_data)
    def test_order(self, goods, expected_result):
        ...
        if product_exists:
            ...
            # 处理商品库存不足的弹窗
            try:
                stock_alert = wait.until(EC.visibility_of_element_located((By.XPATH, "//*[@class='layui-layer-content layui-layer-padding']")))
